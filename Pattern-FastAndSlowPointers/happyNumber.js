...
// function for calculating the square sum of a given number.
...
//     For N>1000, suppose the number has ‘M’ digits and the next number is ‘N1’. From the above Wikipedia link, we know that the sum of the squares of the digits of ‘N’ is at most 92M9^2M9​2​​M, or 81M81M81M (this will happen when all digits of ‘N’ are ‘9’).

// This means:

//     N1<81MN1 < 81MN1<81M
//     As we know M=log(N+1)M = log(N+1)M=log(N+1)
//     Therefore: N1<81∗log(N+1)N1 < 81 * log(N+1)N1<81∗log(N+1) => N1=O(logN)N1 = O(logN)N1=O(logN)

// This concludes that the above algorithm will have a time complexity of O(logN)O(logN)O(logN).
