...
let canPartitionRecursive = function (dp, num, sum, currentIndex, dp) {
  ...
  //   if we have not processed a similar problem we process it
  if (typeof dp[currentIndex][sum] === 'undefined') {
    if (num[currentIndex] <= sum) {
      if (
        ...
        //   we memoize the return value
        dp[currentIndex][sum] = true;
        return true;
      }
    }

    // we also memoize the alternative option where we skip the current value
