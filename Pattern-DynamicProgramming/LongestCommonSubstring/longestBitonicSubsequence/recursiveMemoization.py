...
def find_LBS_length(nums):
    dpLis = [[-1 for _ in range(len(nums)+1)] for _ in range(len(nums))]
    dpLisr = [[-1 for _ in range(len(nums)+1)] for _ in range(len(nums))]
    maxLength = 0
    for i in range(len(nums)):
        longest1 = lis(nums,i,-1,dpLis)
        longest2 = lisr(nums,i,-1,dpLisr)
        maxLength = max(maxLength, longest1 + longest2 - 1)
    return maxLength

def lis(nums,current,previous,dp):
    if current == len(nums):
        return 0
    ...
    if dp[current][previous + 1] == -1:
        ...
        if previous == -1 or nums[current] < nums[previous]:
            c1 = 1 + lis(nums,current+1,current,dp)
        
        c2 = lis(nums,current+1,previous,dp)

        dp[current][previous + 1]=  max(c1,c2)
    return dp[current][previous+1]

def lisr(nums,current,previous,dp):
    if current < 0:
        return 0
    ...
    if dp[current][previous + 1] == -1:
        c1 = 0
        if previous == -1 or nums[current] < nums[previous]:
            c1 = 1 + lisr(nums,current-1,current,dp)
        
        c2 = lisr(nums,current-1,previous,dp)
